...
#
# def legend(price=False, qty=False, profit=False):
#     result = calculate(transactions)
#     x_val = [value[0] for value in result]
#     y_val_price = [value[1] for value in result]
#     y_val_qty = [value[2] for value in result]
#     y_val_current_profit = [value[3] for value in result]
#     y_val_total_profit = [value[4] for value in result]
#     y_val_stock_profit = [value[5] for value in result]
#     if price:
#         plt.plot(x_val, y_val_price)
#     if qty:
#         plt.plot(x_val, y_val_qty)
#     if profit:
#         plt.plot(x_val, y_val_current_profit, label="Current profit")
#         plt.plot(x_val, y_val_total_profit, label='total profit')
#         plt.plot(x_val, y_val_stock_profit, label='stock_profit')
#     plt.legend(loc="upper left")
#     plt.show()
#
#
...
#
# print(history_info)
# simulate_trans = trade_simulate(8, 100, 100, 0.01, 10)
# for dummy_i in simulate_trans:
#     print( simulate_trans[0], simulate_trans[1], simulate_trans[2], simulate_trans[3])
#     values = trade_strategy(history_info, simulate_trans[0], simulate_trans[1], simulate_trans[2], simulate_trans[3])
#     print(values)
...
#     # times是交易次数
#     times = len(tran)
#     # 最终返回交易次数，库存成本，库存股数量，当前损益，累计损益和浮盈
#     stock_values = []
#     # 买入的操作单据建立一个数列，用于计算持仓成本，每次卖出时，计算损益使用上一次计算的持仓成本
#     qty_buy = []
#     price_buy = []
#     qty_sell = [0]
#     price_sell = []
...
#         profit = 0
#         if tran[i][1] > 0:
#             price_buy.append(tran[i][0])
#             qty_buy.append(tran[i][1])
#             amount_buy.append(tran[i][0] * tran[i][1])
#             cost = sum(amount_buy)/sum(qty_buy)
#         else:
#             qty_sell.append(tran[i][1])
#             price_sell.append(tran[i][0])
#             amount_sell.append(tran[i][0] * tran[i][1])
#             profit = (tran[i][0] - cost) * abs(tran[i][1])
#             total_profit += profit
#
#         #计算浮盈时，采用平均交易价格减去持仓成本；
#
#         trade_price = (sum(amount_buy) + sum(amount_sell)) / (sum(qty_sell) + sum(qty_buy))
#         print(trade_price)
#         stock_profit = (trade_price - cost) * (sum(qty_buy)+sum(qty_sell))
#         stock_values.append((i+1, cost, sum(qty_buy)+sum(qty_sell), profit, total_profit, stock_profit, price_buy[-1]))
#     return stock_values
#
#
# def simulate(trans, qty_rate=0.01, price_difference=0.01):
#     history = calculate(trans)
#     stock_qty = history[-1][2]
#     price_buy = history[-1][6]
#     trade_qty = [stock_qty * qty_rate, stock_qty * qty_rate*2, stock_qty*qty_rate*3]
#     trade_price = []
#     for trade_times in range(100):
#         trade_price.append(price_buy + price_difference * (trade_times+1))
#     situation_one = []
#     situation_two = []
#     situation_three = []
#     for price in trade_price:
#         situation_one.append((price, trade_qty[0]))
#         situation_two.append((price, trade_qty[1]))
#         situation_three.append((price, trade_qty[2]))
#     result = calculate(trans + list(situation_one[0]))
#     print(result)
#     trade_profit = result[3]
#     stock_profit = result[5]
#     print(trade_profit + stock_profit)
