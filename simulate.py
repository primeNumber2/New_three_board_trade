...
# 本页脚本列出的策略generate_simulation_tran（hist_tran, days, qty_ratio, price_ratio, price_diff）
#   输入参数为 历史交易数据，策略持续时间，交易数量占总库存的比例，卖出价格与收盘价之比 以及 价差
#   策略为同一买卖价格，同一交易数量，持续特定时长；
...
def generate_simulation_data(file_name, stock_ratio, selling_price_diff, buying_price_diff, days):
    # stock_ratio: 交易数量占总库存的比例； selling_price_diff: 卖出价格和收盘价之比； buying_price_diff：价差； days: 策略持续时间；
...
if __name__ == "__main__":
    ...
    b = calculate_profit(FILE_NAME, a, 1)
